...
import AvatarPicker, { TenantAvatar } from '@/components/common/AvatarPicker'
...
function getProfileState(user) {
  ...
  return {
    ...
    avatar: user?.avatar ?? tenant?.avatar ?? DEFAULT_AVATAR,
  }
}
...
function mergeProfileIntoUser(baseUser, profile) {
  ...
  if (profile?.avatar) nextUser.avatar = profile.avatar
  if (profile?.tenant?.avatar || profile?.avatar) nextTenant.avatar = profile?.tenant?.avatar || profile?.avatar
    ...
    <div className="space-y-5 animate-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="glass rounded-2xl p-6 shadow-lg flex flex-col items-center text-center">
          {/* Clickable avatar — opens picker modal */}
          <button
            type="button"
          ...
          >
            ...
            />
            <span className="absolute inset-0 rounded-xl flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {showAvatarModal && (
            ...
            >
              <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in">
                {/* Modal header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700/60">
                  <div>
                    <h3 className="font-semibold text-sm text-slate-800 dark:text-white">Choose Avatar</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Select a preset avatar for your profile</p>
                  </div>
                  <button
                    type="button"
                ...
                {/* Current preview */}
                <div className="flex items-center gap-3 px-5 py-3 bg-slate-50 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-700/60">
                  <TenantAvatar src={form.avatar || DEFAULT_AVATAR} name={form.name} size="md" />
                  <div>
                    <p className="text-xs font-medium text-slate-700 dark:text-slate-200">Current avatar</p>
                    <p className="text-[10px] text-slate-400 font-mono truncate max-w-[200px]">{form.avatar || DEFAULT_AVATAR}</p>
                  </div>
                </div>
                ...
                <div className="p-5">
                  <AvatarPicker
                    value={form.avatar || DEFAULT_AVATAR}
                    onChange={(path) => {
                ...
                <div className="px-5 pb-4 text-center">
                  <p className="text-[10px] text-slate-400">
                    Click an avatar to select it. Save your profile to apply.
                  </p>
                </div>
              </div>
            </div>
