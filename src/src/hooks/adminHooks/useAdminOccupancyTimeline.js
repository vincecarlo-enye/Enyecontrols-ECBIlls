...
import { fetchAdminOccupancyTimeline, getAdminOccupancyTimelineSnapshot } from '@/services/adminService/adminOccupancyTimelineService'
...
export function useAdminOccupancyTimeline() {
  ...
  const initialSnapshot = getAdminOccupancyTimelineSnapshot({ month: getCurrentMonthValue() })
  const hasInitialSnapshot = initialSnapshot != null
  const [data, setData] = useState(() => normalizeResponse(initialSnapshot))
