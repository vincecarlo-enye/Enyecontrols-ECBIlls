...
export default function UnitFilterBar({ showTimeRange = false }) {
  ...
  return (
    <div
      ...
      className="mb-1 flex flex-col gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-3.5 dark:border-blue-700/50 dark:bg-blue-900/20 lg:flex-row lg:items-center lg:justify-between"
    >
      {showUnitFilter ? (
      ...
      {showTimeRange ? (
        <div className="flex w-full flex-wrap items-center justify-start gap-1.5 sm:justify-end lg:ml-auto lg:w-auto">
          <div className="flex flex-wrap items-center gap-1.5">
            {TENANT_TIME_RANGE_OPTIONS.map((option) => (
              <button
                key={option.value}
                onClick={() => setSelectedTimeRange(option.value)}
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${selectedTimeRange === option.value ? 'bg-blue-600 text-white shadow-sm' : 'text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-800/40'}`}
              >
                {option.label}
              </button>
            ))}
