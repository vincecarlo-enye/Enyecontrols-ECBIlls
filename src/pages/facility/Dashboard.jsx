...
export default function FacilityDashboard() {
  ...
  const { reload: reloadMonitoring } = monitoring
  const { reload: reloadConsumption } = consumption
  const { reload: reloadMaintenance } = maintenance
  const { reload: reloadEquipment } = equipment

  useEffect(() => {
    const timer = window.setInterval(() => {
      reloadMonitoring({ silent: true })
      reloadConsumption({ silent: true })
      reloadMaintenance({ silent: true })
      reloadEquipment({ silent: true })
    }, DASHBOARD_READ_REFRESH_MS)

    return () => window.clearInterval(timer)
  }, [reloadConsumption, reloadEquipment, reloadMaintenance, reloadMonitoring])
  ...
  const summaryTop = [
    {
      title: 'Monitored Floors',
      value: monitoring.floorData.length.toLocaleString(),
      icon: Building2,
      gradient: 'from-blue-500 to-indigo-600',
      glow: 'shadow-blue-500/25',
      badge: `${activeAlerts.length} flagged`,
      badgeUp: activeAlerts.length > 0,
    ...
    },
    {
      ...
      badge: `${equipment.stats.offline} offline`,
      badgeUp: equipment.stats.offline > 0,
    ...
    },
    {
      ...
      badge: `${maintenance.stats.open} open`,
      badgeUp: maintenance.stats.open > 0,
    ...
    },
    {
      ...
      badge: monitoring.anomaly?.severity === 'alert' ? 'Critical anomaly' : 'Monitoring normal',
      badgeUp: monitoring.anomaly?.severity === 'alert',
  ...
  const summaryToday = [
    {
      ...
      badge: `${Math.round(monitoring.currentLoad)}% system load`,
      badgeUp: monitoring.trend > 0,
    ...
    },
    {
      ...
      badge: `${activeAlerts.filter((row) => row.status === 'high').length} floors high`,
      badgeUp: activeAlerts.filter((row) => row.status === 'high').length > 0,
      sub: 'Rolling 24h estimate',
    },
    {
      ...
      glow: 'shadow-rose-500/25',
      badge: monitoring.anomaly?.severity === 'alert' ? 'Above average' : 'Stable',
      badgeUp: monitoring.anomaly?.severity === 'alert',
  ...
  return (
    <div className="section-gap animate-in min-w-0 pb-4">
      <PageSection>
        <PageHeader
          title="Facility Dashboard"
          subtitle="Real-time building operations and utility monitoring."
          icon={Activity}
          actions={(
            <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
              ...
              <div className="flex w-full overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 sm:w-auto">
      ...
      <SummaryCardStrip stretch cards={summaryTop} />
      <SummaryCardStrip stretch cards={summaryToday} />
      ...
      <PageSection>
        <div className="grid min-w-0 grid-cols-1 gap-4 xl:grid-cols-5">
          <ChartCard className="xl:col-span-2" title="Thermal Energy" subtitle={`${filter} thermal trend`} accentHex="#f43f5e">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                <defs>
          ...
          <ChartCard className="xl:col-span-3" title="Utility Comparison" subtitle={`${filter} comparison`} action={<BarChart3 className="w-4 h-4 text-slate-400" />}>
      ...
      <PageSection padded={false}>
        <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-md dark:border-slate-700/50 dark:bg-slate-900">
      ...
      <PageSection>
        <div className="grid min-w-0 grid-cols-1 gap-4 xl:grid-cols-2">
          <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-md dark:border-slate-700/50 dark:bg-slate-900 flex flex-col">
            ...
            <div className="p-5 space-y-3">
              ...
              {activeAlerts.length === 0 ? (
                ...
                <div key={row.floor} className="flex items-center justify-between rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 px-4 py-3 gap-3">
                  <div>
