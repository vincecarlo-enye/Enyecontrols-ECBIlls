  ...
  return (
    <div className="space-y-5 animate-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            Read complete notification details and filter updates by unread status or keywords.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {[
          { label: 'Total Notifications', value: summary.total, tone: 'text-slate-800 dark:text-white' },
          { label: 'Unread', value: summary.unread, tone: 'text-blue-600 dark:text-blue-400' },
          { label: 'Read', value: summary.read, tone: 'text-emerald-600 dark:text-emerald-400' },
        ].map((card) => (
          <div key={card.label} className="rounded-2xl border border-slate-200/70 bg-white px-4 py-4 shadow-sm dark:border-slate-700/60 dark:bg-slate-900">
            <p className="text-[11px] font-mono uppercase tracking-widest text-slate-400">{card.label}</p>
            <p className={`mt-2 text-2xl font-display font-700 ${card.tone}`}>{card.value}</p>
          </div>
        ))}
      </div>
