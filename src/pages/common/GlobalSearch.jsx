...
const NAV_SEARCH_ITEMS = {
  admin: [
    ...
    { path: '/admin/billing-concerns', label: 'Billing Concerns', keywords: ['tickets', 'concerns'] },
  ...
  super_admin: [
    ...
    { path: '/super-admin/billing-concerns', label: 'Billing Concerns', keywords: ['tickets', 'concerns'] },
...
function takeRows(payload, limit = 50) {
  return normalizeRows(payload).slice(0, limit)
}
...
async function fetchRoleSearchResults(role, query) {
  ...
  if (role === 'tenant') {
    const [billsRes, notificationsRes] = await Promise.all([
      fetchTenantBills({ search: query, per_page: 25 }),
      fetchNotifications({ per_page: 25, status: 'all', search: query }),
    ])

    const results = [
      ...buildPageMatches(role),
      ...takeRows(billsRes, 40).map((bill) => mapBillResult(bill, '/tenant/bills')),
      ...takeRows(notificationsRes, 25).map((notification) => mapNotificationResult(notification, notificationPath)),
    ]
  ...
  if (role === 'finance') {
    const [billsRes, tenantsRes, paymentsRes, notificationsRes] = await Promise.all([
      fetchFinanceBills({ search: query, per_page: 25 }),
      fetchFinanceTenants({ search: query, per_page: 25 }),
      fetchFinancePaymentsSearch({ search: query, per_page: 25 }),
      fetchNotifications({ per_page: 25, status: 'all', search: query }),
    ])

    const results = [
      ...buildPageMatches(role),
      ...takeRows(billsRes, 40).map((bill) => mapBillResult(bill, '/finance/billing')),
      ...takeRows(tenantsRes, 40).map((tenant) => mapTenantResult(tenant, '/finance/billing')),
      ...takeRows(paymentsRes, 40).map((payment) => mapPaymentResult(payment, '/finance/payment-review')),
      ...takeRows(notificationsRes, 25).map((notification) => mapNotificationResult(notification, notificationPath)),
    ]
  ...
  if (role === 'facility_manager') {
    ...
    const results = [
      ...buildPageMatches(role),
      ...takeRows(ticketsRes, 40).map((ticket) => mapMaintenanceResult(ticket, '/facility/maintenance')),
      ...takeRows(notificationsRes, 25).map((notification) => mapNotificationResult(notification, notificationPath)),
    ]
  ...
  const concernPath = role === 'super_admin' ? '/super-admin/billing-concerns' : '/admin/billing-concerns'

  const [billsRes, tenantsRes, unitsRes, concernsRes, notificationsRes] = await Promise.all([
    fetchAdminBills({ paginate: 1, per_page: 25, search: query }),
    fetchAdminTenants({ search: query, per_page: 25 }),
    fetchAdminUnits({ search: query, per_page: 25 }),
    fetchAdminBillingConcerns({ search: query, per_page: 25 }),
    fetchNotifications({ per_page: 25, status: 'all', search: query }),
  ])

  const results = [
    ...buildPageMatches(role),
    ...takeRows(billsRes, 40).map((bill) => mapBillResult(bill, billingPath)),
    ...takeRows(tenantsRes, 40).map((tenant) => mapTenantResult(tenant, tenantPath)),
    ...takeRows(unitsRes, 40).map((unit) => mapUnitResult(unit, unitPath)),
    ...takeRows(concernsRes, 40).map((concern) => mapConcernResult(concern, concernPath)),
    ...takeRows(notificationsRes, 25).map((notification) => mapNotificationResult(notification, notificationPath)),
  ]
