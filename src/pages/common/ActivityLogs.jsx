...
export default function ActivityLogsPage() {
  ...
  const timelineCapableCount = useMemo(
    () => logs.filter((item) => item.entity_type && item.entity_id).length,
    [logs]
  )
  ...
  return (
    <div className="space-y-5 animate-in">
      <PageSection>
        <PageHeader
          title="Activity Logs"
          subtitle="Review account actions, approvals, updates, and other recorded activity."
          icon={Activity}
          actions={(
            <button
        ...
        cards={[
          {
            ...
            sub: isGlobalView ? 'Across accessible roles' : 'For your account activity',
            gradient: 'from-slate-500/15 via-slate-400/10 to-transparent',
          },
          {
            title: 'Current Page',
            value: logs.length,
            sub: `Page ${meta.current_page || 1} of ${meta.last_page || 1}`,
            gradient: 'from-sky-500/20 via-cyan-400/10 to-transparent',
          },
          {
            title: 'Timeline Ready',
            value: timelineCapableCount,
            sub: 'Rows with deeper approval history',
