...
import { AlertTriangle, CalendarRange, Download, RefreshCw } from 'lucide-react'
...
function exportCsv(data) {
  const rows = [
    ['Section', 'Label', 'Scope', 'Electricity', 'Water', 'Thermal', 'Variance', 'Variance %'],
    ...Object.entries(data.summary || {}).map(([key, item]) => [
      'Summary',
      key,
      item?.status || '',
      key === 'electricity' ? item?.main_total ?? '' : '',
      key === 'water' ? item?.main_total ?? '' : '',
      key === 'thermal' ? item?.main_total ?? '' : '',
      item?.variance ?? '',
      item?.variance_percent ?? '',
    ]),
    ...Array.isArray(data.pageBreakdown)
  ...
  link.href = url
  link.download = `reconciliation-${data.period?.month || 'report'}.csv`
  link.click()
      ...
      <div className="glass rounded-2xl p-5 shadow-lg">
        ...
        {(data.pageBreakdown || []).length === 0 && (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
            ...
            No page breakdown data found for the selected month yet.
          </div>
        )}
      </div>
    </div>
