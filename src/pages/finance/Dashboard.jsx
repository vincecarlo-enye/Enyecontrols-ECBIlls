...
function normalizeUtilitySummary(payload = {}) {
  const mapCard = (card = {}, fallbackUnit = '') => ({
    ...
    period: card?.period ?? 'Last 7 days',
  })

  return {
    electric: mapCard(payload?.electric ?? payload?.electricity ?? {}, 'kWh'),
    water: mapCard(payload?.water ?? {}, 'm3'),
    thermal: mapCard(payload?.thermal ?? {}, 'kBTU/h'),
  }
}

function normalizeUtilityDaily(payload = {}) {
  const series = {
    electric: Array.isArray(payload?.electric ?? payload?.electricity) ? (payload?.electric ?? payload?.electricity) : [],
    water: Array.isArray(payload?.water) ? payload.water : [],
    thermal: Array.isArray(payload?.thermal) ? payload.thermal : [],
  }

  const dates = new Set([
    ...series.electric.map((row) => row?.date),
    ...series.water.map((row) => row?.date),
    ...series.thermal.map((row) => row?.date),
  ].filter(Boolean))

  const byKey = (rows = []) => new Map(rows.map((row) => [row?.date, row]))
  const electricMap = byKey(series.electric)
  const waterMap = byKey(series.water)
  const thermalMap = byKey(series.thermal)

  return Array.from(dates)
    .sort()
    .map((date) => ({
      date,
      day: electricMap.get(date)?.day || waterMap.get(date)?.day || thermalMap.get(date)?.day || formatDate(date),
      electricity: Number(electricMap.get(date)?.usage ?? 0),
      water: Number(waterMap.get(date)?.usage ?? 0),
      thermal: Number(thermalMap.get(date)?.usage ?? 0),
    }))
}

function buildUtilityBars(rows = [], key) {
...
export default function FinanceDashboard() {
  ...
  const [utilityDaily, setUtilityDaily] = useState([])
  ...
  const loadDashboard = useCallback(async (options = {}) => {
    const { silent = false } = options
      ...
      try {
        ...
        setError('')
        const [billsRes, paymentsRes, utilitySummaryRes, utilityDailyRes] = await Promise.all([
          api.get('/api/finance/bills', createFreshRequestConfig()),
          api.get('/api/finance/payments', createFreshRequestConfig()),
          api.get('/api/finance/dashboard/utilities-summary', createFreshRequestConfig()),
          api.get('/api/finance/dashboard/utilities-daily', createFreshRequestConfig()),
        ])
        ...
        setUtilitySummary(normalizeUtilitySummary(utilitySummaryRes?.data?.data || {}))
        setUtilityDaily(normalizeUtilityDaily(utilityDailyRes?.data?.data || {}))
      } catch (err) {
        setBills([])
        setPayments([])
        setUtilitySummary({
  ...
  const {
    ...
    const totalElec = Number(utilitySummary?.electric?.usage ?? 0)
    const totalWater = Number(utilitySummary?.water?.usage ?? 0)
    const totalThermal = Number(utilitySummary?.thermal?.usage ?? 0)
    ...
    const summaryCards = [
      ...
      },
      {
        ...
        value: `${(totalElec + totalWater + totalThermal).toLocaleString('en-US', { maximumFractionDigits: 2 })}`,
        icon: Zap,
        gradient: 'from-amber-500 to-orange-500',
        shadow: 'shadow-amber-500/25',
        trend: totalElec + totalWater + totalThermal > 0 ? `${Math.round((totalElec / (totalElec + totalWater + totalThermal)) * 100)}% electricity share` : '0% electricity share',
        trendUp: true,
        sub: 'Last 7 days from approved usage',
      },
      {
    ...
    const utilityMeters = {
      electric: {
        usage: totalElec,
        unit: utilitySummary?.electric?.unit || 'kWh',
        estimatedCost: Number(utilitySummary?.electric?.estimatedCost ?? 0),
        trend: Number(utilitySummary?.electric?.trend ?? 0),
      },
      water: {
        usage: totalWater,
        unit: utilitySummary?.water?.unit || 'm3',
        estimatedCost: Number(utilitySummary?.water?.estimatedCost ?? 0),
        trend: Number(utilitySummary?.water?.trend ?? 0),
      },
      thermal: {
        usage: totalThermal,
        unit: utilitySummary?.thermal?.unit || 'kBTU/h',
        estimatedCost: Number(utilitySummary?.thermal?.estimatedCost ?? 0),
        trend: Number(utilitySummary?.thermal?.trend ?? 0),
      },
    }
  ...
  const rangedUtilityRevenue = useMemo(
    () => utilityDaily.slice(-Math.max(3, Math.min(rangeLimit * 2, utilityDaily.length || 7))),
    [utilityDaily, rangeLimit],
  )
  ...
  const loadingState = (pageLoading && bills.length === 0 && payments.length === 0 && utilityDaily.length === 0) || (loading && bills.length === 0 && payments.length === 0 && utilityDaily.length === 0 && !error)
  ...
  return (
    <PageSection variant="light" className="min-w-0">
      <PageHeader
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3 sm:gap-4 mb-4">
        <UtilityCard type="electric" {...utilityMeters.electric} bars={buildUtilityBars(utilityDaily, 'electricity')} />
        <UtilityCard type="thermal" {...utilityMeters.thermal} bars={buildUtilityBars(utilityDaily, 'thermal')} />
        <UtilityCard type="water" {...utilityMeters.water} bars={buildUtilityBars(utilityDaily, 'water')} />
      </div>
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-5 mb-4">
        <ChartCard className="xl:col-span-3" title="Utility Consumption Breakdown" subtitle="Actual meter usage by utility type" action={<BarChart3 className="w-4 h-4 text-slate-400" />}>
          <div className="mb-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
            {[
              ...
              <div key={label} className={`rounded-xl p-3 text-center ${bg}`}>
                ...
                <p className={`text-sm font-bold ${color}`}>{Number(value || 0).toLocaleString('en-US', { maximumFractionDigits: 2 })}</p>
              </div>
            ))}
          </div>
        ...
        <ChartCard className="xl:col-span-2" title="Consumption Distribution" subtitle="Utility share of actual metered usage" action={<PieIcon className="w-4 h-4 text-slate-400" />}>
