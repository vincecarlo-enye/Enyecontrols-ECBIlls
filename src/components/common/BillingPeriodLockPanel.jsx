...
export default function BillingPeriodLockPanel({
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-slate-200/70 dark:border-slate-700/50 bg-white dark:bg-slate-900 p-4 shadow-sm">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-medium ${isLocked ? 'bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-900/20 dark:text-amber-300 dark:border-amber-800' : 'bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800'}`}>
          {isLocked ? <ShieldAlert className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[200px_1fr_auto]">
        <input
          ...
          className="px-3 py-2 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-slate-700 dark:text-slate-200 outline-none focus:border-violet-400"
        />
        <input
          type="text"
          value={reason}
          ...
          className="px-3 py-2 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-slate-700 dark:text-slate-200 placeholder-slate-400 outline-none focus:border-violet-400"
        />
        <button
          onClick={handleSubmit}
          disabled={!billingMonth || saving || loading || (reasonRequired && !reason.trim())}
          className={`inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed ${isLocked ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-violet-600 hover:bg-violet-700'}`}
        >
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-700">
              <th className="px-3 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-slate-400">Month</th>
              <th className="px-3 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-slate-400">Status</th>
              <th className="px-3 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-slate-400">Locked By</th>
              <th className="px-3 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-slate-400">Locked At</th>
              <th className="px-3 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-slate-400">Reason</th>
            </tr>
          </thead>
          <tbody>
            {(activeLocks || []).slice(0, 6).map((item) => (
              <tr key={item.id} className="border-b border-slate-100 dark:border-slate-800">
                <td className="px-3 py-3 font-medium text-slate-700 dark:text-slate-200">{formatMonthLabel(item.billingMonth)}</td>
      ...
      {activeLocks.length === 0 && !loading && (
        <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 px-4 py-3 text-sm text-slate-400">
