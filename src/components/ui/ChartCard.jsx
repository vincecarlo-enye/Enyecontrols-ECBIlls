export default function ChartCard({
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 border-b border-slate-100 px-4 pb-4 pt-5 dark:border-white/6 sm:flex-row sm:items-start sm:justify-between sm:px-5">
        <div className="min-w-0">
          <h3 className="flex min-w-0 items-center gap-2 break-words text-[14px] font-semibold leading-snug text-slate-800 dark:text-slate-100">
            {(accent || accentHex) && (
              <span
      ...
      <div className={noPad ? '' : 'min-w-0 px-4 pb-5 pt-4 sm:px-5'}>
        {children}
      </div>
    </div>
  )
