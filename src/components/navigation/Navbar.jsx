...
export default function Navbar({ onMenuClick }) {
  ...
  return (
    <header className={`h-16 min-w-0 border-b flex items-center px-3 sm:px-4 lg:px-6 gap-2 sm:gap-3 transition-colors backdrop-blur-xl ${isSuperAdmin ? 'glass border-violet-200/60 dark:border-violet-700/40' : 'glass border-slate-200/60 dark:border-slate-700/50'}`}>
      <button onClick={onMenuClick} className="flex-shrink-0 rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700/60 lg:hidden">
      ...
      <div className="hidden min-w-0 items-center gap-2 md:flex">
        <div>
          ...
          <p className="truncate text-[11px] text-slate-400 dark:text-slate-500 font-mono">{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
      </div>

      <div className="hidden min-w-0 flex-1 max-w-md md:block lg:mx-0 lg:ml-4 xl:ml-8">
        <div ref={searchRef} className="relative">
          ...
          />
          {showSearchResults && (
            <div className="absolute left-0 right-0 top-full z-50 mt-2 max-h-[60vh] overflow-hidden overflow-y-auto rounded-2xl border border-slate-200/60 glass shadow-2xl shadow-slate-200/60 dark:border-slate-700/50 dark:shadow-black/40">
      ...
      <div className="ml-auto flex min-w-0 flex-shrink-0 items-center gap-1 sm:gap-2">
        {showUnitFilter && (
          <div className="hidden items-center gap-1.5 rounded-xl border border-blue-200 bg-blue-50 px-3 py-1.5 dark:border-blue-700/50 dark:bg-blue-900/20 md:flex">
        ...
        <button onClick={toggleTheme} className="rounded-xl p-2 text-slate-500 transition-all hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-700/60 dark:hover:text-slate-200" title={isDark ? 'Light mode' : 'Dark mode'}>
          {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
        </button>

        <div ref={notifRef} className="relative">
          <button onClick={() => { setShowNotifs((value) => !value); setShowProfile(false) }} className="relative rounded-xl p-2 text-slate-500 transition-all hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700/60">
        ...
        <div ref={profileRef} className="relative">
          <button onClick={() => { setShowProfile((value) => !value); setShowNotifs(false) }} className="flex items-center gap-2 rounded-xl py-1 pl-1 pr-1 transition-all hover:bg-slate-100 dark:hover:bg-slate-700/60 sm:pr-2">
          ...
          {showProfile && (
            <div className="fixed left-3 right-3 top-16 z-50 animate-in overflow-hidden rounded-2xl border border-slate-200/60 glass shadow-2xl shadow-slate-200/60 dark:border-slate-700/50 dark:shadow-black/40 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-56">
